...
> **注意:** ハブのポートはデフォルトで `8443` です。`--port` で変更可能です。ウェブクライアントには **Hub Port** 入力フィールド（デフォルト: `8443`）が追加されたため、`--port` を変更した場合はそちらで合わせてください。`index.html` の直接編集は不要です。ハブサーバーのホスト名は `window.location.hostname` から自動取得されるため、ウェブサーバーとハブサーバーが同一マシンで動作している場合は URL の手動入力は不要です。別マシンの場合は `index.html` の `baseUrl` を直接編集してください。
...
ウェブクライアントの URL と固有のセッション ID を全参加者に共有します。セッション ID は 64文字以内にしてください。
...
WebTransport 対応ブラウザでウェブクライアントの URL を開き、セッション ID（64文字以下であること）・表示名（任意、`/` を含まず 64文字以下であること）・Hub Port（デフォルト: `8443`、`--port` に合わせてください）・Bridge Port（デフォルト: `8080`、`--ws-port` に合わせてください）を入力して **Connect All** をクリックします。接続が完了するとクライアント ID と表示名が表示されます。接続が切断された場合は指数バックオフ（1秒〜30秒）で自動再接続します。
...
- **OSC Bundle**（timetag の信頼性を保証するため、常にストリームで送信）
