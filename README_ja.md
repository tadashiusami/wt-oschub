...
WebTransport 対応ブラウザでウェブクライアントの URL を開き、セッション ID・表示名（任意）・Bridge Port（デフォルト: `8080`、`bridge.js` 起動時の `--ws-port` に合わせてください）を入力して **Connect All** をクリックします。接続が完了するとクライアント ID と表示名が表示されます。接続が切断された場合は指数バックオフ（1秒〜30秒）で自動再接続します。
