...
WebTransport 対応ブラウザでウェブクライアントの URL を開き、セッション ID・表示名（任意、`/` を含まないこと）・Hub Port（デフォルト: `8443`、`--port` に合わせてください）・Bridge Port（デフォルト: `8080`、`--ws-port` に合わせてください）を入力して **Connect All** をクリックします。接続が完了するとクライアント ID と表示名が表示されます。接続が切断された場合は指数バックオフ（1秒〜30秒）で自動再接続します。
